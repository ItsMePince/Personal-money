...
type AccountItem = { id: string; label: string; type: ItemType; favorite: boolean; favoritedAt?: number; };
...
const filterLabel: Record<FilterKey, string> = { ALL:"ทั้งหมด", CASH:"เงินสด", BANK:"ธนาคาร", CREDIT:"บัตรเครดิต" };
...
export default function AccountSelect() {
  ...
  const toggleFavorite = (id: string) => {
    setItems(prev => {
      const now = Date.now();
      return prev.map(it =>
        it.id === id ? { ...it, favorite: !it.favorite, favoritedAt: !it.favorite ? now : undefined } : it
      );
    });
  };
  ...
  const sortedAndFiltered = useMemo(() => {
    const byFilter = (it: AccountItem) =>
      filter === "ALL" ? true : filter === "CASH" ? it.type === "cash" : filter === "BANK" ? it.type === "bank" : it.type === "credit";
  ...
  return (
    <div className="screen">
      ...
      <div className="dropdown">
        <button className="dropdown__button" onClick={() => setMenuOpen(v=>!v)} aria-haspopup="listbox" aria-expanded={menuOpen}>
        ...
        {menuOpen && (
          <ul className="dropdown__menu" role="listbox" onMouseLeave={() => setMenuOpen(false)}>
            {(["ALL","CASH","BANK","CREDIT"] as FilterKey[]).map((k,i)=>(
              <li key={k} role="option" aria-selected={filter===k}
                  className={`dropdown__option ${i>0?"with-sep":""}`}
                  onClick={()=>{ setFilter(k); setMenuOpen(false); }}>
                {filterLabel[k]}
              </li>
            ))}
          </ul>
